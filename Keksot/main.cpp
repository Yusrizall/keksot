...
void checkSwitch()
{
    for (int i = 0; i < 5; ++i)
    {
        for (int j = 0; j < 4; ++j)
        {
            ...
            if (i > 0 && !table[i - 1][j].empty() && !all_of(currentPlate.cakes.begin(), currentPlate.cakes.end(), [](int v) { return v == 0; }) && table[i - 1][j][0].cakes != currentPlate.cakes && !all_of(table[i - 1][j][0].cakes.begin(), table[i - 1][j][0].cakes.end(), [](int v) { return v == 0; }))
            {
                cout << "Possible switch exists for plate (" << i << "," << j << ") with plate (" << i - 1 << "," << j << ")" << endl;
            }
            if (i < 4 && !table[i + 1][j].empty() && !all_of(currentPlate.cakes.begin(), currentPlate.cakes.end(), [](int v) { return v == 0; }) && table[i + 1][j][0].cakes != currentPlate.cakes && !all_of(table[i + 1][j][0].cakes.begin(), table[i + 1][j][0].cakes.end(), [](int v) { return v == 0; }))
            {
                cout << "Possible switch exists for plate (" << i << "," << j << ") with plate (" << i + 1 << "," << j << ")" << endl;
            }
            if (j > 0 && !table[i][j - 1].empty() && !all_of(currentPlate.cakes.begin(), currentPlate.cakes.end(), [](int v) { return v == 0; }) && table[i][j - 1][0].cakes != currentPlate.cakes && !all_of(table[i][j - 1][0].cakes.begin(), table[i][j - 1][0].cakes.end(), [](int v) { return v == 0; }))
            {
                cout << "Possible switch exists for plate (" << i << "," << j << ") with plate (" << i << "," << j - 1 << ")" << endl;
            }
            if (j < 3 && !table[i][j + 1].empty() && !all_of(currentPlate.cakes.begin(), currentPlate.cakes.end(), [](int v) { return v == 0; }) && table[i][j + 1][0].cakes != currentPlate.cakes && !all_of(table[i][j + 1][0].cakes.begin(), table[i][j + 1][0].cakes.end(), [](int v) { return v == 0; }))
            {
                cout << "Possible switch exists for plate (" << i << "," << j << ") with plate (" << i << "," << j + 1 << ")" << endl;
            }
        }
